...
namespace Supdate.Web.App.Controllers
{
  [RoutePrefix("manage")]
  [Authorize]
  public class ManageController : AuthenticatedControllerBase
  {
    ...
    */
    ...
    {
      var emailAddress = String.IsNullOrWhiteSpace(CurrentUser.UnConfirmedEmail)
        ? CurrentUser.Email
        : CurrentUser.UnConfirmedEmail;
      return View("_editEmail", new ChangeEmailModel{EmailAddress = emailAddress});
    }

    [Route("profile")]
    public ActionResult EditProfile()
    {
      return View(new ProfileViewModel { Passwords = new ChangePasswordViewModel(), User = CurrentUser });
    }
    ...
    public ActionResult EditProfile(ProfileViewModel model)
    {
      ...
      UserManager.SaveProfile(HttpContext.GetOwinContext(), user);
      this.SetNotificationMessage(NotificationType.Success, "Your profile has been updated.");
    ...
    public async Task<ActionResult> ChangePassword(ProfileViewModel model)
    {
      if (!ModelState.IsValid)
      {
        return View("EditProfile", new ProfileViewModel { Passwords = model.Passwords, User = CurrentUser });
      }

      var result = await UserManager.ChangePasswordAsync(User.Identity.GetUserId<int>(), model.Passwords.OldPassword, model.Passwords.NewPassword);
      if (result.Succeeded)
      {
        var user = await UserManager.FindByIdAsync(User.Identity.GetUserId<int>());
        if (user != null)
        ...
        this.SetNotificationMessage(NotificationType.Success, "The password was changed successfully.");
      ...
      {
        this.SetNotificationMessage(NotificationType.Error, error);
      }
